//! Scatter plot trace for the MapLibre `map` subplot.
...
///
/// `ScatterMap` is the modern MapLibre-based counterpart to
/// [`ScatterMapbox`](crate::ScatterMapbox): it draws markers and/or lines from
/// `lat`/`lon` coordinates onto the `layout.map` subplot.
///
/// # Examples
///
/// ```
/// use plotly::ScatterMap;
///
/// let trace = ScatterMap::new(vec![45.5017], vec![-73.5673]).name("montreal");
/// ```
#[serde_with::skip_serializing_none]
#[derive(Serialize, Clone, Debug, FieldSetter)]
#[field_setter(box_self, kind = "trace")]
pub struct ScatterMap<Lat, Lon>
where
    Lat: Serialize + Clone,
    Lon: Serialize + Clone,
{
    #[field_setter(default = "PlotType::ScatterMap")]
    r#type: PlotType,
    ...
    /// Sets a reference between this trace's data coordinates and a `map`
    /// subplot. If "map" (the default value), the data refer to `layout.map`.
    ...
    /// Determines if this scattermap trace's layers are to be inserted before
    /// the layer with the specified ID. By default, scattermap layers are
    /// inserted above all the base layers. To place the scattermap layers
...
impl<Lat, Lon> ScatterMap<Lat, Lon>
where
...
impl<Lat, Lon> Trace for ScatterMap<Lat, Lon>
where
    Lat: Serialize + Clone,
    Lon: Serialize + Clone,
{
...
mod tests {
    ...
    fn serialize_scatter_map() {
        ...
        assert_eq!(to_value(scatter_map).unwrap(), expected);
    }
}
