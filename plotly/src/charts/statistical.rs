    ...
    H: num::Num + Serialize,
{
    ...
    pub fn text(mut self, text: Dim<String>) -> Box<Histogram<H>> {
        self.text = Some(text);
        Box::new(self)
    }

    pub fn hover_text(mut self, hover_text: Dim<String>) -> Box<Histogram<H>> {
        self.hover_text = Some(hover_text);
        Box::new(self)
    }

    pub fn hover_info(mut self, hover_info: &str) -> Box<Histogram<H>> {
        self.hover_info = Some(hover_info.to_owned());
        Box::new(self)
    }

    pub fn hover_template(mut self, hover_template: Dim<String>) -> Box<Histogram<H>> {
